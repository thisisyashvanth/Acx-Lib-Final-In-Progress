...
# For Swagger -> In OAuth2 Spec
def login_form(form_data: UserLoginReq, db: Session):
...
def employee_signup(user: UserSignupReq, db: Session):
    isExistingEmail = db.query(User).filter(User.email == user.email).first()
    isExistingEmployeeId = db.query(User).filter(User.employee_id == user.employee_id).first()
